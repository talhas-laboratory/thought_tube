...
from conversation_os.feed_certification_harness import (
...
PROBE_SUITE = FIXTURES_DIR / "feed_certification_probes.json"


class FeedCertificationHarnessTestCase(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_positive_probe_returns_pairs_with_provenance(self) -> None:
        seed_certification_corpus(self.root)
        suite = json.loads(PROBE_SUITE.read_text(encoding="utf-8"))
        probe = next(row for row in suite["probes"] if row["probe_id"] == "positive-research-evidence-pairs")
        result = evaluate_positive_probe(self.root, probe)
        self.assertEqual(result["verdict"], "pass")
        self.assertGreater(result["pair_count"], 0)
        self.assertTrue(result["has_provenance"])
        self.assertIn("research-insight", result["source_slugs"])

    def test_bridge_parity_matches_bridge_retrieval_subset(self) -> None:
        seed_certification_corpus(self.root)
        suite = json.loads(PROBE_SUITE.read_text(encoding="utf-8"))
        probe = next(row for row in suite["probes"] if row["probe_id"] == "bridge-parity-research-domain")
        result = evaluate_bridge_parity_probe(self.root, probe)
        self.assertEqual(result["verdict"], "pass")
        self.assertTrue(result["parity_ok"])
        self.assertEqual(result["bridge_source_refs"], result["pair_source_refs"])

    def test_negative_probe_returns_no_pairs(self) -> None:
        seed_certification_corpus(self.root)
        suite = json.loads(PROBE_SUITE.read_text(encoding="utf-8"))
        probe = next(row for row in suite["probes"] if row["probe_id"] == "negative-unrelated-domain")
        report = run_feed_certification_suite(self.root, {"probes": [probe]}, seed=False)
        self.assertEqual(report["results"][0]["verdict"], "no_pairs")
        self.assertEqual(report["results"][0]["pair_count"], 0)

    def test_run_suite_generates_machine_readable_report(self) -> None:
        report = run_feed_certification_suite(self.root, PROBE_SUITE)
        ...
        self.assertIn("bridge-parity-research-domain", summary)
    ...
    def test_fixture_revision_is_versioned(self) -> None:
        ...
        self.assertTrue(str(suite.get("fixture_revision", "")).startswith("cae-feed-cert-v2"))
