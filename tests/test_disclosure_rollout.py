...
class DisclosureRolloutTestCase(unittest.TestCase):
    ...
    def test_release_runtime_enforces_primary_surfaces(self) -> None:
        release_runtime = json.loads(
            (Path(__file__).resolve().parents[1] / "product" / "inner_world_v1" / "config" / "runtime.json").read_text(
                encoding="utf-8"
            )
        )
        self.assertEqual(release_runtime["bridge"]["disclosure_rollout_v1"], "enforced")
        self.assertEqual(release_runtime["holodeck"]["disclosure_rollout_v1"], "enforced")
        self.assertTrue(release_runtime["bridge"]["disclosure_service_v1"])
        self.assertTrue(release_runtime["holodeck"]["disclosure_service_v1"])
