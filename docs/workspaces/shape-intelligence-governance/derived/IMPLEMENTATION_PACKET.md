...
Validation checks schema, source/segment evidence existence and digest, allowed transitions, agent authorization, idempotency key, budget, and policy version. It does not determine whether cited evidence semantically supports a Shape. Receipt stores service-assigned request/candidate/evaluation IDs, packet fingerprint, agent/model/prompt/tool versions, timing, retry/cost facts, validation outcome, and redaction-safe provenance.

## State and failure rules

Allowed lifecycle: `proposed → under_review → recommended | rejected | needs_evidence`; no automatic `canonical`. A human approval is an immutable authorization event, not a model-settable candidate state. Replayed idempotency key returns the original receipt. Any validation, persistence, or receipt failure rolls back all writes. Candidate operations cannot touch retrieval indexes or canonical projections.
